...
async def list_talk_rooms(current_user: dict = Depends(get_current_user)):
    idols = (
        await supabase_client.supabase.table("idols").select("id, name, photo_url").order("name").execute()
    ).data

    async def _room_for(idol):
        last_broadcast_result, last_private_result = await asyncio.gather(
            supabase_client.supabase.table("idol_broadcasts")
            .select("content, media_type, created_at")
            .eq("idol_id", idol["id"])
            .order("created_at", desc=True)
            .limit(1)
            .execute(),
            supabase_client.supabase.table("idol_private_messages")
            .select("content, media_type, created_at")
            .eq("idol_id", idol["id"])
        ...
        candidates = last_broadcast_result.data + last_private_result.data
    ...
    rooms = await asyncio.gather(*(_room_for(idol) for idol in idols))
    rooms = sorted(rooms, key=lambda r: r["lastMessageAt"] or "", reverse=True)
    return {"rooms": rooms}


@router.get("/talks/{idol_id}/messages")
async def get_talk_messages(idol_id: str, current_user: dict = Depends(get_current_user)):
    broadcasts_result, private_messages_result = await asyncio.gather(
        supabase_client.supabase.table("idol_broadcasts").select("*").eq("idol_id", idol_id).order("created_at").execute(),
        supabase_client.supabase.table("idol_private_messages")
        .select("*")
        .eq("idol_id", idol_id)
        .eq("producer_id", current_user["sub"])
    ...
    broadcasts = broadcasts_result.data
    private_messages = private_messages_result.data
...
async def send_talk_message(
    ...
    idol_result, producer_result = await asyncio.gather(
        supabase_client.supabase.table("idols").select("*").eq("id", idol_id).execute(),
        supabase_client.supabase.table("producers").select("first_name, username").eq("id", current_user["sub"]).execute(),
    )
