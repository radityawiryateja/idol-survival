...
async def _compute_real_progress(validation_type: str, producer_id: str, target_count: int, today: str):
...
async def get_or_create_today_missions(producer_id: str) -> list[dict]:
    today = _today()
    templates = (
        await supabase_client.supabase.table("mission_templates").select("*").order("sort_order").execute()
    ).data

    existing = (
        await supabase_client.supabase.table("producer_missions")
        .select("*")
        .eq("producer_id", producer_id)
    ...
    for template in templates:
        ...
        real_progress = await _compute_real_progress(
